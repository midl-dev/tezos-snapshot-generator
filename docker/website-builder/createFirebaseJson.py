...
firebase_conf["hosting"]["redirects"] = [ { "source": ":block.rolling", "type": 301, "destination": "https://storage.googleapis.com/%s/:block.rolling" % os.environ['WEBSITE_BUCKET_URL'].replace("gs://", "") },
        ...
        { "source": "rolling", "type": 301, "destination": "https://storage.googleapis.com/%s/tezos-%s-%s.rolling" % (os.environ['TEZOS_NETWORK'], os.environ['WEBSITE_BUCKET_URL'].replace("gs://", ""), os.environ['BLOCK_HEIGHT']) },
        { "source": "full", "type": 301, "destination": "https://storage.googleapis.com/%s/tezos-%s-%s.full" % (os.environ['TEZOS_NETWORK'], os.environ['WEBSITE_BUCKET_URL'].replace("gs://",""), os.environ['BLOCK_HEIGHT']) } ]
